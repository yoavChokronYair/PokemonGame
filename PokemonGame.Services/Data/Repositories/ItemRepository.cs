...
namespace PokemonGame.Services.Data.Repositories
{
    ...
    internal class PlayerUsedTmRepository
    {
        private readonly IDbConnectionService _db;

        internal PlayerUsedTmRepository(IDbConnectionService db)
        {
            _db = db;
        }

        public bool HasUsedTm(int playerId, int tmHmId) =>
            _db.QuerySingle<int>(
                @"SELECT COUNT(*)
                  FROM PlayerUsedTms
                  WHERE PlayerID = @playerId
                  AND TmHmId = @tmHmId",
                new { playerId, tmHmId }) > 0;

        public List<PlayerUsedTmData> GetByPlayer(int playerId) =>
            _db.Query<PlayerUsedTmData>(
                @"SELECT
                    PlayerID,
                    TmHmId
                  FROM PlayerUsedTms
    ...
    internal class RegisteredKeyItemRepository
    {
        private readonly IDbConnectionService _db;

        internal RegisteredKeyItemRepository(IDbConnectionService db)
        {
            _db = db;
        }

        public RegisteredKeyItemData? GetByPlayer(int playerId) =>
            _db.QuerySingle<RegisteredKeyItemData?>(
                @"SELECT
                    PlayerID,
                    KeyItemId
                  FROM RegisteredKeyItems
                  WHERE PlayerID = @playerId",
                new { playerId });
    }
    ...
    internal class PlayerItemGiftRepository
    {
        private readonly IDbConnectionService _db;

        internal PlayerItemGiftRepository(IDbConnectionService db)
        {
            _db = db;
        }

        public PlayerItemGiftData? Get(int playerId, int itemGivingId) =>
            _db.QuerySingle<PlayerItemGiftData?>(
                @"SELECT
                    PlayerID,
                    ItemGivingId,
                    HasBeenGiven
                  FROM PlayerItemGifts
                  WHERE PlayerID = @playerId
                  AND ItemGivingId = @itemGivingId",
                new { playerId, itemGivingId });

        public List<PlayerItemGiftData> GetByPlayer(int playerId) =>
            _db.Query<PlayerItemGiftData>(
                @"SELECT
                    PlayerID,
                    ItemGivingId,
                    HasBeenGiven
