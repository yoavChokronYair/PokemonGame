...
namespace PokemonGame.ViewModels.ViewModelPage.Online
{
    ...
    // ── Main OnlineBattleViewModel ────────────────────────────────────────────
    public class OnlineServerBattleViewModel : ViewModelBase
    {
        private readonly UserStore _userStore;
        private readonly OnlineBattleService _onlineService;
        private readonly SyncService? _syncService;

        // ── Child VMs (bound by PokemonBattlePage) ────────────────────────────
        public OnlineBattleStatusViewModel PlayerStatus { get; } = new();
        public OnlineBattleStatusViewModel EnemyStatus { get; } = new();
        public OnlinesBattleMenuViewModel BattleMenu { get; }
        public OnlineBattleLoggerViewModel Logger { get; } = new();

        // ── Extra observables ─────────────────────────────────────────────────
        private string _winnerName = string.Empty;
        public string WinnerName { get => _winnerName; set => SetProperty(ref _winnerName, value); }

        public ICommand ForfeitCommand { get; }

        public OnlineServerBattleViewModel(UserStore userStore)
        {
            _userStore = userStore;
            _onlineService = userStore.OnlineBattleService
                             ?? throw new InvalidOperationException("OnlineBattleService not set.");
            _syncService = userStore.SyncService;

            BattleMenu = new OnlinesBattleMenuViewModel(OnMoveChosen, Logger);

            _onlineService.OnMatchFound += HandleMatchFound;
            _onlineService.OnTurnResult += HandleTurnResult;
            _onlineService.OnForcedSwitch += HandleForcedSwitch;
            _onlineService.OnBattleEnd += HandleBattleEnd;
            _onlineService.OnError += HandleError;

            ForfeitCommand = new AsyncRelayCommand(SendForfeitAsync);
        }
        ...
        private void HandleMatchFound(MatchFoundPacket packet)
        {
            RunOnUi(() =>
            {
                ...
                PlayerStatus.PokemonName = "Your Pokémon";
                PlayerStatus.MaxHP = packet.PlayerMaxHp;
                PlayerStatus.CurrentHP = packet.PlayerMaxHp;
                PlayerStatus.Level = packet.PlayerLevel;
                PlayerStatus.PokedexId = packet.PlayerPokedexId;

                EnemyStatus.PokemonName = packet.RivalName;
                EnemyStatus.MaxHP = packet.EnemyMaxHp;
                EnemyStatus.CurrentHP = packet.EnemyMaxHp;
                EnemyStatus.Level = packet.EnemyLevel;
                EnemyStatus.PokedexId = packet.EnemyPokedexId;

                BattleMenu.LoadMoves(packet.PlayerMoveNames);
        ...
        private void HandleTurnResult(TurnResultPacket packet)
        {
            RunOnUi(() =>
            {
                PlayerStatus.CurrentHP = packet.PlayerHp;
                PlayerStatus.StatusCondition = packet.PlayerStatusCondition;
                PlayerStatus.PokedexId = packet.PlayerPokedexId;

                EnemyStatus.CurrentHP = packet.EnemyHp;
                EnemyStatus.StatusCondition = packet.EnemyStatusCondition;
                EnemyStatus.PokedexId = packet.EnemyPokedexId;

                Logger.EnqueueLines(packet.LogLines);
                BattleMenu.SetActionsEnabled(true);
            });
        }

        private void HandleForcedSwitch(ForcedSwitchPacket packet)
        {
            RunOnUi(() =>
            {
                BattleMenu.SetActionsEnabled(false);
                Logger.Show("Your Pokémon fainted — choose a replacement!");
                // TODO: show switch UI — wire AvailableSlots if you add a switch panel
            });
        }
        ...
        private void HandleBattleEnd(BattleEndPacket packet)
        {
            ...
            {
                BattleMenu.SetActionsEnabled(false);
                WinnerName = packet.WinnerName == _userStore.Username ? "You won!" : $"{packet.WinnerName} wins!";
                Logger.Show(WinnerName);

                if (_syncService != null)
                {
                    int battleId = await _syncService.PushBattleResultAsync().ConfigureAwait(false);
                    await _syncService.PushParticipantAsync(new BattleParticipantData
                    {
                        BattleID = battleId,
                        BattlePlayerID = _userStore.BattlePlayerID,
                    ...
                    {
                        BattleID = battleId,
                        BattlePlayerID = packet.OpponentBattlePlayerId,
        ...
        private static void RunOnUi(Func<Task> asyncAction) =>
            RunOnUi(() => { _ = asyncAction(); });
    }
}