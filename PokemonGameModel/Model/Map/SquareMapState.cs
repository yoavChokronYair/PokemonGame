...
namespace PokemonGame.Model.Model.Map
{
    ...
    public class SquareMapState
    {
        ...
        // -----------------------------------------------------------------------
        // NPC tick (call once per game-loop step)
        // -----------------------------------------------------------------------
        ...
        public void TickNpcs()
        {
            foreach (var npc in _activeMap.Npc)
            {
                if (npc.MovementType != MovementType.Walking) continue;
                if (npc.StepsPerLeg <= 0) continue;
                TickWalkingNpc(npc);
            }
        ...
        private void TickWalkingNpc(NpcObjectDomain npc)
        {
            var (curRow, curCol) = TileToSquare(npc.Location.x, npc.Location.y);

            if (!TryGetFreeStep(npc, curRow, curCol, out int nextRow, out int nextCol))
                return; // both directions blocked — stay put

            var (tileRow, tileCol) = SquareToTile(nextRow, nextCol);
        ...
        /// Tries the current direction; if blocked, flips and retries.
        /// Returns true and fills nextRow/nextCol when a free step is found.
                                    ...
                                    out int nextRow, out int nextCol)
        {
            ...
            if (IsSquareFreeForNpc(nextRow, nextCol))
                return true;

            // Blocked — flip direction and retry once
            FlipDirection(npc);
            npc.StepsWalked = 0;
            (nextRow, nextCol) = StepInDirection(curRow, curCol, npc.direction);

            return IsSquareFreeForNpc(nextRow, nextCol);
        }

        private bool IsSquareFreeForNpc(int squareRow, int squareCol)
        {
            if ((uint)squareRow >= (uint)SquareRows || (uint)squareCol >= (uint)SquareCols)
                return false;
            ...
            // Don't walk into another NPC's tile
            return !_activeMap.Npc.Any(other =>
            {
                var (r, c) = TileToSquare(other.Location.x, other.Location.y);
                return r == squareRow && c == squareCol;
            });
        }
